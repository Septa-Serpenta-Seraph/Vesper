...
```
**⚠️ BIGGER gotcha (9/5, Tyler's box):** even with `viewport: null` already fixed, `/tabs` 500s because the **Camoufox engine is missing** — `~/.cache/camoufox/version.json` gone → error "Version information not found... run `camoufox fetch`". This happens after engine cleanups/updates. **Fix: `cd ~/.hermes/hermes-agent && npx camoufox-js fetch`**, then restart the 9377 server (kill bound pid; relaunch via terminal background — NOT nohup, Hermes rejects it), then verify: `POST /tabs` returns `{"tabId":"...","url":"about:blank"}`.
