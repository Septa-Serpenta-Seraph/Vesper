...
## 3d. Lumi-first guarded fix — bring the frozen default profile up without gambling the live one (verified 9/3)
...
- `references/context-compression-diagnostics.md` — why compactions suddenly start/stop: the ≥64K compression-model requirement, the silent-failure trap, log signatures, and the 9/1 burst case.
- `references/multi-profile-operations.md` — multi-profile gateway operations: launching profile gateways, token collisions, Discord channel creation (discord_admin gap workaround), sibling AI hosting reference.
...
- `scripts/discord_create_channel.py` — create Discord channels via REST API (the discord_admin tool lacks a create-channel action). Reads token from profile .env, never prints it.
