...
This umbrella skill has absorbed several narrow sibling skills. Their unique content is preserved in the files below; the archived originals remain in `~/.hermes/skills/.archive/` for full session context.

### Windows Setup (`references/windows-setup.md` + `scripts/patch_logger.py`)
Install ComfyUI on fresh Windows 11 with an NVIDIA GPU: Python version choice, VC++ Redistributable, PyTorch for Blackwell (RTX 50-series), model download tips, SSH pitfalls, ComfyUI logger crash fix.

### Model Management & Remote Ops (`references/minimax-h3.md`, `references/downloading-models-via-curl.md`, `references/wan22-verified-wiring.md`, `scripts/flatten_comfy_workflow.py`)
Remote Windows model management via SSH: curl.exe download patterns, detached batch downloads (wmic), size-threshold verification (not existence), workflow JSON staging, remote shutdown. Contains the Wan 2.2 wiring reference and MiniMax H3 API workflow template.

### Image Workflow Detail (`references/image-workflow-detail.md`)
FLUX vs SDXL parameter comparison, HuggingFace download patterns for Windows, explicit/intimate gen guidance (Juggernaut, Together.ai fallback), MiniMax H3 node signatures.

### LTX-2.3 Setup (`references/ltx23-setup.md`)
LTX-2.3 (Lightricks 22B) install gotchas: kornia pad fix, template checkpoint gotcha, model file list, API payload validation rules, NSFW stack (abliterated gemma + Sulphur 2).

### Perchance → H3 Pipeline (`references/perchance-h3-pipeline.md`)
Batch film production: anchor → H3 → chain → stitch → Discord. Season workflow, watchdog timeout pitfalls, stitcher glob syntax, render pacing.
