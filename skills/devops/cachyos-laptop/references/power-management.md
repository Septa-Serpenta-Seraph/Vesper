...
## SSH access (set up 2026-09-04; key + UFW fixed 2026-09-10)

- From the gateway box: `ssh -i ~/.ssh/cachyos_laptop tyler@192.168.0.34`
  (LAN-only). 9/10: UFW was silently blocking port 22 (`ufw-reject-input`,
  policy DROP) despite sshd active — fixed with `sudo ufw allow ssh` on the
  laptop. IP may change on DHCP — re-discover with `hostname -I` if it stops.
...
- **DE = KDE Plasma on Wayland** (`wayland-0`, plasmashell). X11 xorg.conf.d
  files are ignored; input config lives in `~/.config/kcminputrc`.

## Trackpad accidental right-click (big palm) — FIXED 2026-09-11

Symptom: palm resting on right side of the large trackpad registers
right-click. Cause: KDE Wayland default `ClickMethod=areas` (corner zones).
Fix (verified in `~/.config/kcminputrc`, device section
`[Libinput][2362][12306][ASUP1207:00 093A:3012 Touchpad]`): add
`ClickMethod=clickfinger` → right-click = **two-finger click only**; corner
palm pressure can no longer right-click. Takes effect on relogin (KDE reads
config at session start). Backup `kcminputrc` before sed edits.
