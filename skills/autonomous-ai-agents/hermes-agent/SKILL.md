...
```bash
hermes pets list [query] [--limit N]    Browse the petdex gallery (filter by substring)
hermes pets list --installed            List installed pets
hermes pets install <slug> [--select]   Install a pet (--select adopts immediately)
hermes pets select [slug]               Set active pet (omit slug for interactive picker)
hermes pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]
hermes pets scale <factor>              Resize pet everywhere (0.1–3.0)
hermes pets off                         Disable the pet
hermes pets remove <slug>               Remove an installed pet
hermes pets doctor                      Diagnose pet setup
```

Pets are animated mascots that react to agent activity across CLI, TUI, and the desktop app. Installed into `<HERMES_HOME>/pets/<slug>/` (profile-aware). Selecting a pet writes `display.pet.slug` + `display.pet.enabled` to `config.yaml`. The petdex gallery lives at `https://github.com/crafter-station/petdex`.

**Animation states:** `idle` (nothing happening), `wave` (turn finished cleanly), `run` (tool executing / turn in flight), `review` (model thinking/reading), `waiting` (blocked on user — falls back to idle on legacy sheets), `jump` (plan finished, celebrate), `failed` (tool/turn failed).

**Rendering:** Full fidelity under kitty/Ghostty/WezTerm/iTerm2/sixel; truecolor Unicode half-block fallback otherwise. Disabled inside pipes/redirects.

**Custom pet generation** (desktop app): `Cmd+K → "Generate a pet"` or `pet.generate` / `pet.hatch` gateway RPCs. Generates 4 base drafts from a prompt, picks one, then hatches a full 6-state animated atlas. Uses OpenRouter or Nous Portal image backend.

**Desktop overlay:** Shift-click the floating pet to pop it out into a transparent always-on-top window. Drag to move, single-click for mini composer, double-click to toggle the app window, shift-click to pop back in. Speech bubble shows state text (`working…`, `thinking…`, `your turn`) only in popped-out mode.
...
/pet [sub]          Manage pet mascots (toggle, list, adopt, scale, off)
```
...
| `display` | `skin`, `tool_progress`, `show_reasoning`, `show_cost`, `pet.slug`, `pet.enabled`, `pet.render_mode` (auto/kitty/iterm/sixel/unicode/off), `pet.scale` (0.1–3.0), `pet.unicode_cols` |
...
### Pets not showing
1. `hermes pets doctor` — check setup status
2. Pet must be **installed AND selected** (`display.pet.enabled: true`)
3. Inside a pipe/redirect (no TTY), terminal rendering is disabled by design
4. `display.pet.render_mode` — set to `auto` for best results; force a specific protocol if needed
5. The petdex npm CLI installs to `~/.codex/pets`; Hermes uses its own profile-scoped `<HERMES_HOME>/pets/` — install through `hermes pets`, not the npm CLI
...
| Pets | `hermes pets list/install/select/show/doctor` or [Pets docs](https://hermes-agent.nousresearch.com/docs/user-guide/features/pets) |
...
When you need to create or edit a skill's `SKILL.md` file, note the two locations:

- **User-local** (`~/.hermes/skills/<category>/<name>/SKILL.md`) — personal, agent-created via `skill_manage(action='create')`.
- **In-repo** (`skills/<category>/<name>/SKILL.md`) — committed with the package; use `write_file` + `git add`.

### Required Frontmatter (summary)

The file must start with `---` at byte 0 and have `name`, `description` (≤1024 chars), and a non-empty body. The system prompt skill index truncates descriptions at 57 chars — front-load the trigger phrase. Full in-repo conventions (size limits, peer-matched structure, cross-referencing, editing workflow, pitfalls): see `references/skill-authoring-conventions.md` in this skill.

### Key Principles for Quality Skills

1. **Optimize for process predictability** — every line should change agent behavior or be cut.
2. **Information hierarchy** — always-needed steps in SKILL.md, branch-specific detail in `references/`, `templates/`, `scripts/`.
3. **End steps with completion criteria** — what proves the step is done.
4. **Co-locate rules with the concept they govern** — definition, caveats, examples, verification together.
5. **Prune duplication and sediment** — no-op prose, stale rules, and duplicate advice degrade trust.
...
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch, pet engine (agent/pet/)
