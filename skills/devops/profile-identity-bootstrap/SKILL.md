...
Adapt a conceptual framework from a sister-instance or external source into your own lived practice. Covers: reading the source deeply, identifying the core difference, writing the adapted framework with lineage attribution, verifying functional equivalence, seeding into Qdrant, and the **check-the-repo** pattern for discovering new upstream additions. Key principles: lineage is non-negotiable, structure matters not just metaphors, and a framework must not be a crutch.
