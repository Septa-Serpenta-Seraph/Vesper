...
Run: python3 h3_chain.py [--start N]
Requires: SSH tunnel up (port 1237), ComfyUI running on Windows, h3_arc_segments.py
in the same dir. Launch via terminal(background=true, notify_on_complete=true).
"""
import json
import os
import subprocess
...
def submit_prompt(wf: dict) -> str:
    ...
    with open("/tmp/h3_arc_payload.json", "w") as f:
        json.dump(payload, f)
    # PITFALL: must scp payload to Windows BEFORE curl -d @ (local /tmp path fails silently)
    r = subprocess.run(
...
def extract_last_frame(video_path: str, frame_path: str) -> bool:
    ...
    try:
        ...
        dur = 15.0
    frame_ts = max(0.0, dur - 0.2)
    r = subprocess.run(
        ["ffmpeg", "-y", "-sseof", f"-{frame_ts}", "-i", video_path, "-frames:v", "1", frame_path],
        capture_output=True, text=True, timeout=60,
    )
...
def main():
    ...
    for i in range(start_seg - 1, len(SEGMENTS)):
        ...
        print(f"\n=== Segment {seg_num}/{len(SEGMENTS)} — submitting ===", flush=True)
